...
function updateStatus(bookingId: number, newStatus: string) {
    ...
    actionForm.patch(`/admin/bookings/${bookingId}/status`, {
        preserveScroll: true,
    });
}
...
function deleteBooking(bookingId: number) {
    if (confirm('Are you sure you want to delete this booking entry?')) {
        actionForm.delete(`/admin/bookings/${bookingId}`, {
            preserveScroll: true,
        });
    }
}
    ...
    <div class="p-6 space-y-6 max-w-7xl mx-auto">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-neutral-500">Monitor, approve, reject, or filter reservations across all courts.</p>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse text-xs">
                    ...
                    <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                        <tr v-for="b in bookings.data" :key="b.id" class="hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition-colors">
                            ...
                            <td class="py-3 px-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button @click="deleteBooking(b.id)" class="p-1 text-rose-400 hover:text-rose-600">
                        ...
                        <tr v-if="bookings.data.length === 0">
                            <td colSpan="9" class="py-8 text-center text-xs text-neutral-400">No bookings found matching filter criteria.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
