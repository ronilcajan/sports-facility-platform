...
function submitUpload() {
    if (!uploadForm.image || !props.venue) return;
    uploadForm.post(`/admin/venues/${props.venue.id}/images`, {
        preserveScroll: true,
        onSuccess: () => {
            clearSelectedFile();
        },
...
function setPrimaryImage(image: VenueImageItem) {
    if (!props.venue) return;
    const actionForm = useForm({});
    actionForm.patch(`/admin/venues/${props.venue.id}/images/${image.id}/primary`, {
        preserveScroll: true,
    });
}

function deleteImage(image: VenueImageItem) {
    if (!props.venue || !props.canDelete) return;
    if (!confirm('Are you sure you want to delete this venue photo?')) return;
    const actionForm = useForm({});
    actionForm.delete(`/admin/venues/${props.venue.id}/images/${image.id}`, {
        preserveScroll: true,
    });
}
                ...
                <div class="rounded-2xl border border-dashed border-neutral-300 dark:border-neutral-700 p-5 bg-neutral-50/50 dark:bg-neutral-850 space-y-4">
                    ...
                    <div v-if="previewUrl" class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                        <label class="flex items-center gap-2 cursor-pointer text-xs font-semibold text-neutral-700 dark:text-neutral-300">
                            ...
                            />
                            <span>Set as primary hero photo</span>
                        </label>
                    ...
                    <div v-if="venue?.images && venue.images.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        >
                            <div class="aspect-video relative overflow-hidden bg-neutral-100 dark:bg-neutral-800">
                                <img :src="img.url" :alt="venue.name" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                                <span
                                    v-if="img.is_primary"
